...
- Like and Retweet the pinned Twitter Post on our Twitter Page


NOTE: Users found cheating would be disqualified & banned immediately.

Airdrop Date: *{AIRDROP_DATE}*<a href="https://bscscan.com/token/0x8e8C11C590aaDc588D9b27D097a8dA76290d02C6">CONTRACT</a>
<a href="https://azarspace.com">WEBSITE URL</a>
"""

MAKE_SURE_TELEGRAM = f"""
🔹 Do not forget to join our Telegram group(s)

<a href="https://t.me/azarspace_en">TELEGRAM GROUP</a>
"""
MAKE_SURE_TELEGRAMC = f"""
🔹 Do not forget to join our Telegram Channel

<a href="https://t.me/Azarspacenews">TELEGRAM CHANNEL</a>
"""
FOLLOW_TWITTER_TEXT = f"""
🔹 Follow our Twitter page(s)

<a href="{TWITTER_LINKS}">TWITTER PAGE</a>


Also Like and Retweet the pined post on the twitter page with #Azarspace #azartoken #BSC:

 <a href="https://twitter.com/Azartoken/status/1502257417853325312?t=nOppjGlDq5iWSVKUGFcVCw&s=19">TWITTER POST</a>
"""
...
def checkCaptcha(update, context):
    ...
    if CAPTCHA_DATA[user.id] != text:
        ...
        update.message.reply_text(text=WELCOME_MESSAGE.replace("NAME", NAME),
                                  reply_markup=ReplyKeyboardMarkup([['🚀 Join Airdrop']]), parse_mode=telegram.ParseMode.HTML)
...
def submit_details(update, context):
    update.message.reply_text(text=PROCEED_MESSAGE, parse_mode=telegram.ParseMode.HTML)
    update.message.reply_text(text="Please click on \"Submit Details\" to proceed", parse_mode=telegram.ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(
...
def follow_telegram(update, context):
    update.message.reply_text(text=MAKE_SURE_TELEGRAM, parse_mode=telegram.ParseMode.HTML)
    update.message.reply_text(text="Please click on \"Done\" to proceed", parse_mode=telegram.ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(
        [["Done"], ["Cancel"]]
    ))

    return FOLLOW_TELEGRAMC
...
def follow_telegramc(update, context):
    update.message.reply_text(text=MAKE_SURE_TELEGRAMC, parse_mode=telegram.ParseMode.HTML)
    update.message.reply_text(text="Please click on \"Done\" to proceed", parse_mode=telegram.ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(
        [["Done"], ["Cancel"]]
    ))
...
def follow_twitter(update, context):
    if not check_joined_channel(user = update.message.from_user.id):
            ...
            return FOLLOW_TWITTER
    update.message.reply_text(text=FOLLOW_TWITTER_TEXT, parse_mode=telegram.ParseMode.HTML)
    update.message.reply_text(text="Type in the link to *your Twitter profile* to proceed.\n\nExample: \nhttps://twitter.com/example", parse_mode=telegram.ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(
        [["Cancel"]]
    ))
...
def submit_address(update, context):
    ...
    update.message.reply_text(text=SUBMIT_BEP20_TEXT, parse_mode=telegram.ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(
        [["Cancel"],["/restart"]]
    ))
...
def end_conversation(update, context):
    ...
    if users.find({"bep20": update.message.text}).count() != 0:
        update.message.reply_text(text="Wallet Address Already Exists. Try again!\n\nExample: \n0xdEAD000000000000000042069420694206942069", parse_mode=telegram.ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(
        [["Cancel"],["/restart"]]
    ))
        return END_CONVERSATION
...
- {joke[1]}
"""
    update.message.reply_text(reply, reply_markup=ReplyKeyboardMarkup(reply_keyboard), parse_mode=telegram.ParseMode.HTML)
    return LOOP
...
PROCEED, FOLLOW_TELEGRAM,FOLLOW_TELEGRAMC, FOLLOW_TWITTER, SUBMIT_ADDRESS,END_CONVERSATION, LOOP, SUREWANTTO, CAPTCHASTATE = range(9)
...
states = {
    ...
    FOLLOW_TELEGRAMC: [MessageHandler(Filters.regex('^Done$'), follow_telegramc), cancelHandler,reset_handler, MessageHandler(Filters.regex(".*"),error_telegram)],
...
def error_start(update,context):
    update.message.reply_text(text="Use /start command to start the Airdrop bot",parse_mode=telegram.ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(
        [["/start"]]
    ))
